...
from probs.test_knowledge.baseline_agents import RandomAgent, UncertaintyAgent, EmpowerAgent, EmpowerOnestepAgent, NoisyEmpowerAgent, Noisyv2EmpowerAgent, CulturalEvolutionAgent, EmpowerCulturalEvolutionAgent
from probs.test_knowledge.LLM_agents import OAAgent, ChatgptAgent, ChatgptCulturalevoAgent, LLama2Agent, LLama2AgentWithCopy
import random
import numpy as np
...
class Group:
    ...
    def _init_agents(self):
        ...
        self.empower_agent = EmpowerAgent(idx=0,
    ...
    def reset_task(self, task, envs):
        ...
        self.empower_agent.reset_task(task, self.envs[idx])
        self.empower_agent.neighbors = []

    def step(self, current_step, empower_actions):
        ...
        for agent in self.agents:
            ...
            if not agent.success:
                ...
                state = ""
                # we add the info from empowerment
                first_word, second_word = empower_actions.split()

                state = state + "What do you get if you combine " +  first_word + " and " + second_word + "?"
            ...
            if agent.success:
                ...
                if len(agent.past_actions):
                    ...
                    items = list(agent.env.wordcraft_env.env.recipe_book.entities)
                    action_0 = items[agent.past_actions[-1][0]]
                    action_1 = items[agent.past_actions[-1][1]]
                    action_words = action_0 + ' ' + action_1
            ...
            group_results.append([self.trial,
             ...
             agent.invalid_attempts,
                                  len(agent.env.wordcraft_env.env.table),copy_time])
