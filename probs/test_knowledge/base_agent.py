...
class Agent:
    ...
    def query_until_valid(self, state, current_step):
        ...
        output = self.query_knowledge(state=state, current_step=current_step)
        print("queried knwoeldege")
        print(output)
    ...
    def parse_action(self, output):
        pos = find_nth(output, "Result: '", 0)
        action = None
        action_str = output[:200]
        error = ""
        if pos != (-1):
            action_str = action_str[pos:]
            pos = find_nth(action_str, "Result '", 0)

            pos = pos + len("Result: '")
            end_comb = find_nth(action_str, "'", 2)
            action_str = action_str[(pos - 1):(end_comb + 1)]
            first_word = action_str[2:-1]
            # convert to wordcraft-compatible action
            #pos = action_str.find('Reasoning ')
            #first_word = action_str[1:(pos - 1)]
            action_str = first_word
            items = list(self.env.wordcraft_env.env.recipe_book.entities)
            action_0  = items.index(first_word)
            action = [action_0, 0]
