        ...
        for step in steps:
            ...
            for trial in trial_values:
                for task in task_values:
                    ...
                    max_values.append(current_results['inventory_length'].max())
            counterx=0
            for trial in trial_values:
                for task in task_values:
                    current_results = step_results.loc[step_results['trial'] == trial]
                    ...
                    try:
                        total_df.loc[len(total_df)] = [trial, step, model_name,  current_results['inventory_length'].mean(), np.mean(max_values[counterx])]
                    except IndexError:
...
def openended():
    ...
    targeted_dirs = [
        "results/papers/social_structure/ce/",
        "results/papers/social_structure/empower/",
        "results/papers/social_structure/chatgpt/",
    ...
    for top_dir in targeted_dirs:
        compare_label(top_dir, trials, parameter="num_agents", parameter_2="connectivity", order=order, label="reward")
