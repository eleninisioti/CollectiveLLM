...
class Agent:
    def __init__(self, idx, project_dir, trial, env_config, retry=6):
    ...
    def create_env(self, seed):
        if self.env_config["openended"]:

            env = gym.make("LittleAlchemy2TextOpen-v0",
                           seed=seed,
                           max_mix_steps=self.env_config["steps"],
                           encoded=self.env_config["encoded"])
            ...
            env = gym.make("LittleAlchemy2TextTargeted-v0",
                           seed=seed,
                           max_mix_steps=self.env_config["steps"],
                           num_distractors=self.env_config["distractors"],
                           max_depth=self.env_config["depth"],
                           encoded=self.env_config["encoded"])
    ...
    def move(self, state=""):
        ...
        first_word, second_word = self._get_action()
        counter = 0
        if self.forbid_repeats:
            already_played = [first_word, second_word] in self.past_actions
        ...
        return output, [first_word, second_word]
    ...
    def log_step(self, step, obs, action):
        with open(self.current_log + "/game.txt", "a") as f:
            ...
            f.write("Attempted action: " + str(action) + "\n \n")
